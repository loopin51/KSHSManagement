import {
  ...
  export async function getAvailableQuantity(equipmentId: string, dateTime: Date): Promise<number> {
    try {
        ...
        console.error(`Error getting available quantity for ${equipmentId}:`, error);
        return 0; // Fail safe: assume no quantity is available.
    }
  }
  ...
  export async function checkRentalCollision(equipmentIds: string[], startDate: Date, endDate: Date): Promise<{ collision: boolean; message: string; conflictingItem?: string }> {
    try {
        ...
        console.error("Error checking rental collision:", error);
        // Fail safe: assume there is a collision to prevent double booking.
        return {
            collision: true,
            message: "대여 가능 여부 확인 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
        };
    }
  }
  ...
  export async function createRental(rentalData: Omit<Rental, 'id' | 'status'>): Promise<Rental> {
    try {
        ...
        throw new Error("데이터베이스에 대여 기록을 생성하는 데 실패했습니다.");
    }
  }